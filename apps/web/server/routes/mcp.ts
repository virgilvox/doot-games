...
* "## rank": players drag items into order; a consensus ranking is shown. Fields: prompt, image, timer (default none). 2+ "- item". Give an item its own picture by putting a pipe and the image URL after the label, e.g. "- Tacos | https://example.com/tacos.jpg" (a hosted URL, or an uploaded one from upload_image). The results then lead with the room's #1 large with that picture and list the rest of the order under it. A pipe NOT followed by an image URL stays part of the label.
* "## tier": the whole room places items into S/A/B/C/D tiers, building one consensus board (the big screen shows it form live). Fields: prompt, image, timer (default none), "tiers: S | A | B | C | D" (pipe-separated band labels), "scored: yes" (score matching the room), "hideboard: yes" (hide until reveal). 2+ "- item" (put a pipe and an image URL after a label for a per-item picture, e.g. "- Pizza | https://example.com/pizza.jpg").
...
Circuit Cypher (robot rap battle), Open Mic, Truth or Share, Quiz or Die, "What, You Didn't Know That?", Story Chain (a pass-it-on writing telephone), Doodle Chain (the draw-and-describe Gartic-Phone telephone), Wavelength (a clue-giver privately sees a hidden point on a spectrum and the room guesses), Bingo (every player gets a unique card and the host calls items live), Call It (host-resolved live predictions), and Tier List (the room votes one item at a time into S-to-F bands and the board fills live; remixable with a prompt deck of items) build their rounds at play time and have bespoke flows. Don't try to write them as markdown; tell the user to open one from the Create page and host it, or call list_game_types. (To author a quick tier round inside a custom game, the "## tier" standalone block still works.)
