/**
 * Spectator streaming over CLASP-signaled WebRTC. The host captures the race
 * renderer's WebGL canvas and serves one peer connection per viewer; CLASP carries
 * only the signaling (offer / answer / ICE), the media flows host->viewer directly.
 * This is a purely additive "watch by room code" layer; gameplay and controller
 * input never touch it (PRD invariant: second-screen, never shared-video).
 *
 * Standalone COPY of the Retro Arcade stream module (no game-to-game import, per the
 * one-game-per-folder isolation rule). Its `setRtcConfig` therefore has its own
 * module-level config, so this game's stream is STUN-only until the app's rtc plugin
 * also configures it (TURN follow-up; see HANDOFF).
 *
 * The relay exposes only a retained `set` (no ephemeral emit), so ICE is sent as an
 * ACCUMULATING ARRAY on one address per direction (the latest set carries every
 ...
 * RTCSessionDescription / RTCIceCandidate expose their fields via prototype getters,
 * so they must be copied to plain objects before publishing.
 *
 * Audio: the game's synth + music plays through Web Audio, so the host taps the
 * master into a MediaStream (PitAudio `getAudioStream`) and folds that track in
...
export function serveStream(
  ...
  // Keep `cached` in sync with the live canvas + audio: recapture if the canvas
  // object ever changes, and fold in the audio track once the game routes sound
  // (which often starts AFTER viewers connect). Returns the current stream.
  ...
  // (no renegotiation), so late-arriving audio reaches viewers already connected. This
  // is what makes a viewer who joined before the game made any sound eventually hear
  // it (their audio m-line was set up empty up front).
  const syncPeers = () => {
