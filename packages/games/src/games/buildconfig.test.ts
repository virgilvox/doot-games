...
describe('every deck-feedable game is self-consistent (meta)', () => {
  ...
  it('covers all 18 deck-fed flagships', () => {
    expect(feedable.map((p) => p.manifest.id).sort()).toEqual(
      ['backronym', 'ballpark', 'faker', 'fib-finder', 'hivemind', 'mad-libs', 'most-likely', 'open-mic', 'over-under', 'quip-clash', 'quiz-or-die', 'sketch-spot', 'split-room', 'tier-list', 'truth-or-share', 'type-the-answer', 'what-you-didnt-know', 'would-you-rather'].sort(),
    )
  })
