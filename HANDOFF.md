...
pnpm test         # 120 tests (+2 live tests, skipped unless DOOT_LIVE=1)
...
The flagship-games phase + the catalog IA + the support/UI polish are **shipped and
live**. Paste the prompt below into a new session to tackle what remains.

> You're picking up **Doot**, a live party-game platform (host on a big screen, players join from phones over the CLASP relay). Live at https://doot.games; repo `virgilvox/doot-games`; trunk `main` (every push to `main` deploys via CI). It's mature: **ten games** incl. three flagship "Games From Doot" (Quip Clash, Mad Libs, Split the Room) on a **two-phase make→judge** engine primitive, the full catalog IA (Explore / Create / Your Games / Home), saved games + auth + uploads, a support page, and 120 tests + 2 live, all deployed.
>
> **Read first, in order:** `HANDOFF.md` (current state), `CLAUDE.md` (invariants, read fully), `docs/authoring-a-game.md` + `examples/` (how games are built), `docs/flagship-games.md` (§4 build order, §6 robustness roadmap), `docs/external-plugins.md` (the external-plugin design), `docs/games-from-doot.md` (official-games/admin/versioning roadmap), `Doot-PRD.md`. Design source of truth: `doot-mockup.html`.
>
> **Hard rules:** no AI attribution in commits (no "Generated with" / `Co-Authored-By`; the word "CLAUDE" only ever refers to the file `CLAUDE.md`); games are **blocks + compositions** (block = round kind with content schema + Player/Host views + `aggregate` + answer-withholding; game = manifest + ordered `{block,content}`; the two-phase flagships compose a make block (quip/fill) with a judge block (vote/split) whose content is derived at runtime; never make one game import another); ephemeral state lives on the relay only (nothing about a live room touches the DB); answers/submissions withheld until reveal; identity is reconnect-by-name; **CSS-first animation** (Pixi only for canvas-heavy bits); accessibility is a build requirement (semantic HTML + ARIA, color paired with shape/label, `prefers-reduced-motion`, an untimed option, screen-reader support on phones); no em dashes in copy or docs. Branch off `main` for work. Verify every change with `pnpm test && pnpm -r typecheck && pnpm --filter @doot-games/web build` (120 tests green; the 2 live tests run with `DOOT_LIVE=1`). Playtest with `scripts/playtest.mjs` (Playwright). Keep `main` deployable; ship small verified increments; **deploy only when the work is done + audited** (merge to `main` → CI deploys; verify `/api/health` + the changed pages after).
>
> **Mission: finish the remaining roadmap.** In rough priority:
>
> 1. ✅ **Circuit Cypher v2 SHIPPED** (the 4th flagship, robot rap battle). First-party Game From Doot (`circuit-cypher`, `mic` icon). Now composes a new guided **`bars` block** (the robot raps you the first line of each couplet, you write the rhyming line back, ×2 → a 4-line verse; `toVoteText` assembles it) → `vote` with the opt-in **`perform`** flag. When `perform` is set, `VoteHost` shows "Perform the bars" + an animated **`RobotRapper`** (`@doot-games/ui` — CSS robot, bobs to a beat, equalizer mouth) that raps each verse via `speechSynthesis` (client-only, SSR-guarded no-op via `speakLines`). The vote-prompt derive keeps the authored prompt for structured make blocks (fill/bars) and only borrows a quip's prompt as the topic. Playtested end-to-end (5/5 scenarios). **The full battle is designed in `docs/flagship-games.md` §8 (next build):** head-to-head **tournament** (two robots face off per matchup; pair until everyone's battled — a custom `components` flow since pairings are dynamic), **live cheers** during a performance over a new continuous relay channel (minor points) + **post-battle 1v1 vote** as decider (`vote` `head-to-head` mode), a **"players perform live"** mode (beat + per-performer timer instead of TTS), cumulative cash scoring, and the **Tone.js generated beat**.
>
> 1b. ✅ **"What, You Didn't Know That?" SHIPPED** (5th flagship, trivia gameshow — `what-you-didnt-know`, `bell` icon). New **`buzzer` block**: escalating-value multiple choice, withheld answer (`redactContent`/`REDACTION_RULES`), client-timed answers (Kahoot-style `ms`, skew-immune), scoring = value + up to +50% speed + 25% buzz bonus for the fastest correct. **`BuzzerHost`** = gameshow stage (glowing value tag, lettered answer panels, live "N locked in", reveal lights the correct panel + crowns the buzz-in). **`BuzzerReveal`** dings (`@doot-games/ui playDing`) + celebrates the first correct on their phone. Everyone plays on phones (works at a panel or at home). Playtested. **Full gameshow designed in `docs/flagship-games.md` §9 (next):** the 4-contestant panel + audience-steal (host-judged, custom `components`), quick-draw tiebreaker (reuse `draw`), final lightning round, specialty content packs (actor photos, a theme-song audio block, Six Degrees, etc.). Also fixed a live bug: GamePlayer now shows "Tap to catch up / Reload" instead of a silent "get ready" stall when a phone's stale bundle (mobile-Safari bfcache) doesn't know a round's block.
>
...
>
> 3. **Robustness roadmap** (`docs/flagship-games.md` §6): **untimed mode** (a PRD §2.5 requirement — a host toggle that drops all round timers and advances when everyone has submitted); the **timeout safety net** (auto-fill an unsubmitted free-text round from a per-prompt pool at 50% points — no dead air, no zeros); **content-filter tiers** (off/moderate/strict + family/adult prompt packs, validated at the relay boundary); **audience-as-discounted-bloc** voting.
>
> 4. **Smaller / known gaps:** own-answer hiding doesn't fire for `fill`/`split` votes (no `.text` field — scoring still excludes self-votes; fix via per-player "your option id" delivery, the reserved `assignment`/`promptFor` path); add `PlayerReveal` to poll/rank/rate/draw (only guess + vote have it); OAuth/magic-link; Postgres for multi-instance; upload hardening (presigned-POST size policy); and the admin role + DB-backed official games + deeper versioning (`docs/games-from-doot.md`, all additive).
>
> **Where things live:** blocks in `packages/games/src/blocks/<kind>/`, the two-phase wiring + scoring + generic renderer in `packages/games/src/runtime/` + `blocks/scoring.ts`, engine + state machine + the runtime-derived-content primitive in `packages/engine/src/` (`room.ts`, `addresses.ts`), the SDK contract in `packages/sdk/src/block.ts`, UI in `packages/ui/src/` (incl. `GameCover`/`GameTypeIcon`/`SiteFooter`/`visuals.ts`), shell/editor/catalog in `apps/web` (`HostRoom.client.vue` wires `deriveContent`/`buildConfig`; `packages/games/src/catalog.ts` is the server-safe list, kept in sync with `registry.ts` by a test). Auth is better-auth (`server/utils/auth.ts`); durable store is libSQL/SQLite (`server/utils/db.ts`).
>
