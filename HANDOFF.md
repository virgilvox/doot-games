...
>
> **Recently shipped (all deployed + verified, do not redo):** the **consumer-bug batch A1-A5** (gameshow guess board + uncropped question image; the live vote/answer distribution is **hidden until reveal** by default with a host "Peek" toggle; **instant-join** roster; honest **upload gating** with hosting still account-free; a **name-collision** prompt at join). **Author/host features B6-B9** (per-answer **subtitle** on guess/buzzer; **scoring is correct-only**, a wrong answer never scores; an optional host-set **player cap** with a "room is full" gate; **co-host/MC delegation**, the host hands the advance controls to a player who drives from their phone). **Circuit Cypher tournament foundations (D13a)**: `buildBracket`/`tallyBattle`/`headToHeadPoints` + a `RobotBattle` two-robot view + a custom relay channel (`room.publishExtra`/`onExtra`). Two bugs fixed en route: the pre-join name-probe latency, and "Host now" doing nothing after an edit.
>
> **Read first, in order:** `HANDOFF.md` (current state - the section above this prompt), `CLAUDE.md` (invariants, read fully), `docs/authoring-a-game.md` + `examples/` (how games are built), `docs/flagship-games.md` (**§8** Circuit Cypher tournament + its **"Build status"** = the exact D13b plan; **§9** the full gameshow; **§6** robustness roadmap), `docs/external-plugins.md`, `docs/games-from-doot.md`, `Doot-PRD.md`. Design source of truth: `doot-mockup.html`.
>
...
> - **Workflow:** branch off `main`. Verify EVERY change with `pnpm test && pnpm -r typecheck && pnpm --filter @doot-games/web build` (~172 tests; the 2 live tests need `DOOT_LIVE=1`). **Playtest with `scripts/playtest.mjs`** (Playwright; drives host + players through the flagships). For UI work, **screenshot at 390px** (phone) and check for 0 horizontal overflow. Keep `main` deployable; **ship small verified increments**; deploy = merge to `main` (CI builds + deploys); after deploy verify `/api/health` + the changed pages on production.
>
> **Remaining work - the full backlog, by priority. Do NOT lose any of these.**
>
> **A. Consumer bugs / polish - ✅ ALL DONE + deployed.** (A1 gameshow guess board + uncropped question image; A2 distribution hidden until reveal + a host "Peek" toggle, default hidden; A3 instant-join roster via profile-seeded presence + a lobby fast path; A4 honest upload gating, hosting confirmed account-free; A5 a pre-join name-collision probe + prompt.) Don't redo.
>
> **B. Author / customization features - ✅ ALL DONE + deployed.** (6 per-answer **subtitle** on guess/buzzer, shown host/phone/reveal; 7 scoring is **correct-only** - a wrong answer never scores; NOTE: the backlog originally read "award points regardless of correctness" but the real intent was the opposite, so that mode was built then **reverted**, see [[doot-scoring-correct-only]] / memory; 8 optional host-set **player cap** with a "room is full" gate, reconnects bypass it; 9 **co-host/MC delegation** - the host delegates the advance controls from the lobby [pick from roster, or off-by-default "first to join"] and the delegate drives from their phone over a replay-safe relay command channel; the host stays the authority and can take back control.) Don't redo.
>
> **C. Social / discovery:**
...
> 13. **Circuit Cypher 1v1 tournament** (`docs/flagship-games.md` §8 + its **"Build status"** subsection = the exact plan). **D13a foundations DONE + shipped** (`buildBracket`/`tallyBattle`/`headToHeadPoints`, the `RobotBattle` two-robot view, and the `room.publishExtra`/`onExtra` battle channel). **D13b is THE NEXT BIG ITEM**: the custom **`CircuitCypherHost`/`CircuitCypherPlayer`** flow - `circuit-cypher.ts` gains `components:{Host,Player}` + a single `bars` write round; on lock, build the bracket from the verses, then host-driven matchups (perform robot A via TTS → perform robot B → A/B head-to-head vote → result) published over the battle channel, finishing with cumulative cash. The engine stays in active/round-0; the battle is custom relay state (NOT engine rounds, the matchup count is dynamic). Reconnect via retained battle state + pid-keyed votes. Then (c) live cheers + bonus, (d) live-perform timer mode, (e) Tone.js beat. **Two-phone playtest required; do NOT wire into the live game until verified end-to-end.**
