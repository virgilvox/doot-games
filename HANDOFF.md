...
>
> 1. ✅ **Circuit Cypher SHIPPED** (the 4th flagship, robot rap battle). It's a first-party included Game From Doot (`circuit-cypher` in `registry.ts`/`catalog.ts`/`visuals.ts`, `mic` icon), composing `fill` (rap verses, `showTemplate` on) → `vote` with a new opt-in **`perform`** flag: when set, `VoteHost` shows a "Perform the bars" control that reads each verse aloud via `speechSynthesis` (client-only, SSR-guarded no-op via `@doot-games/ui` `speakLines`) over a CSS beat, then the room votes. Playtested end-to-end. **Still to layer on (follow-ups, not blockers):** the `vote` block's **`head-to-head` mode** (today field-only — pair submissions, vote per matchup, tally wins) and the **Tone.js generated-beat fallback** for when no `musicUrl` is set (a `RoomMeta.musicUrl`/SFX/`AudioController` + mute is the broader audio layer).
>
