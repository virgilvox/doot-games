...
| `@doot-games/games` | Done + tested. Blocks (guess/rate/poll/rank/**draw**/**quip**/**vote**/**fill**), the generic renderer, the **two-phase derive wiring** (`toVoteText`/`DeriveSource.render`) + a pure tested **`scoring.ts`** (vote-share, multiplier, sweep/pity, closeness, speed-decay), **nine games** (Guess, Rate, Poll, Rank, Draw, VoteBox, **Quip Clash** + **Mad Libs** flagships, **Custom**), and a **markdown game parser** (`parseMarkdownGame`). |
