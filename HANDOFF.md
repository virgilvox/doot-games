...
> **THE TWO OPEN DECISIONS, DECIDED (2026-09-07). SHIPPED + DEPLOYED as `769873c`
> (CI `34171534169`, all four jobs green).**
...
> SHIPPED + DEPLOYED as `bd64289` (CI `34165696015`, all four jobs green).**
...
Paste the prompt below into a new session. Everything above this line is the running
log, newest first; the prompt is the short version plus the gotchas that cost real time.

> You're picking up **Doot**, a self-hostable platform for **live party games**: a host puts a game on a big screen, a crowd joins from their phones with a 4-char code or QR, and everyone plays in real time over the **CLASP** relay (`wss://relay.clasp.to`). Live at **https://doot.games**; repo `virgilvox/doot-games`; trunk `main`, and **every push to `main` deploys to production via CI** (check -> build -> deploy -> a post-deploy smoke against prod). There is no staging.
>
> **State:** mature and deployed. **38 games**, **36 blocks**, **1065 tests** plus 6 opt-in live ones, every package typechecks (including the stricter `nuxi typecheck`), and the Nuxt app builds.
>
> **The most important lens: the primary user is a non-developer at a party, not a developer.** Judge every change by "what happens to a regular person hosting or playing this in a room full of people?"
>
> **READ THIS FIRST, it is the freshest and most load-bearing context.** In September a real event ran a 145-phone room and **scored nobody**: the final board read `0 / 14` for all 93 players, people who had answered correctly were told "Not quite", and "Lock it in" was greyed out on phones. Two independent faults, neither of which any test could see, because both were architecture rather than logic. The whole top of `HANDOFF.md` is that story and the rework it caused. The short version, which is now invariant:
> - **Presence is measured from ARRIVAL, never from a timestamp somebody else's machine wrote.** Heartbeats are relay EVENTS (`relay.emit`) carrying no payload: never stored, never replayed. Comparing a remote clock to yours is what greyed out a whole room when one venue laptop ran 25s slow. **Never reintroduce a cross-machine clock comparison for liveness.**
> - **Presence must never GATE an action, only annotate one.** Submitting is not gated on the host being present; a round's auto-advance waits on a named SET of players that presence can shrink only when the host explicitly removes someone. Being wrong about someone who left costs a few seconds of dead air; being wrong about someone still playing costs them the question.
> - **The host is the single writer of the roster** (`/doot/<room>/roster`), like phase/round/config. Only the host subscribes to `player/*/ping`. That took per-phone heartbeat traffic from O(N) to O(1): measured at 0.2 frames/s in a 70-player room, the host's own beat and nothing else.
> - **Presence is unpaced**: one cadence (5s), one window (20s), every room. A party behaving differently from the room you tested is the shape of the original failure.
> - Read `docs/clasp-primer.md` before touching any of it.
>
> **Read next, in order:** `HANDOFF.md` (the log above this prompt), `CLAUDE.md` (invariants, read fully), `docs/architecture.md`, `docs/authoring-a-game.md` + `examples/`, `docs/flagship-games.md`, `Doot-PRD.md`. **`docs/BACKLOG.md` is dated 2026-06-01 and predates this entire cycle** - treat it as historical, not as a to-do list, until someone re-verifies it against the code.
>
> **Hard rules:**
> - **Commits:** no AI attribution. No "Generated with", no `Co-Authored-By` trailers, no tool credits. The word "CLAUDE" only ever refers to the file `CLAUDE.md`. This overrides any default trailer behaviour.
> - **Copy:** **no em dashes** anywhere user-facing (commas, colons, periods, spaced hyphens); **no emojis in UI**, use the `Icon` component from `@doot-games/ui`; no third-party brand names in product copy; the Create page heading is **"Create"**.
> - **Architecture:** games are **blocks + compositions**. A block is a round kind (content schema + Player/Host views + `aggregate` + answer withholding); a game is a manifest plus an ordered list of `{block, content}`. Two-phase flagships compose a make block with a judge block derived at runtime. A **custom-flow** game overrides `components: {Host, Player}` and drives its own state over `/x/<key>` channels (`publishExtra`/`onExtra`). **No game imports another. Nothing about a live room touches the DB. Answers are withheld until reveal.** Identity is reconnect-by-name (`p_<hash(room+name)>`). The host is the sole authority; CLASP has no ACL, the engine enforces rules in code.
> - **Animation:** CSS first. Pixi only for the 2D draw canvas, Three only for the rap-battle arena, both lazy + client-only + SSR-guarded. Audio is client-only and a clean no-op where unavailable. Honour mute and `prefers-reduced-motion`.
> - **Accessibility is a build requirement:** semantic HTML + ARIA, colour paired with shape or label, `prefers-reduced-motion`, an untimed option, screen-reader support on phones.
> - **Workflow:** verify every change with `pnpm test && pnpm -r typecheck`, and for UI also `pnpm --filter @doot-games/web build`. Ship small verified increments. Deploy is a push to `main`; afterwards check `/api/health` and the changed pages **on production**, not just locally.
>
> **Gotchas that cost real time. These are the expensive ones:**
> - **A green unit test is not a working feature, and it will cover for a wrong belief about the system.** This has now happened three times here: the editor's section drag shipped completely dead while 47 ordering tests passed, because nothing ever performed a drag; a results podium clipped its 7th row with every test green, found only by opening `/dev/results` and looking; and a "kick" fix passed its test for days because the test fed the function a hand-written roster instead of the real one, which the UI could not even produce. **When a change is about a gesture, drive the gesture. When it is about what a screen shows, look at the screen.**
> - **`/dev/results` is the gallery of every shape the results board must handle.** A page-overflow check reports 0 while a section is clipped inside its own panel, so measure `slide.scrollHeight - slide.clientHeight` there, never `documentElement.scrollWidth`. Distributions PAGE (`runtime/slides.ts`); the leaderboard and awards CAP at 8 with a "+N more", which is deliberate and not the same defect.
> - **Auto-advance only misbehaves with TIMERS OFF.** With a timer running, the engine's own deadline closes the round first and any test of auto-advance is measuring nothing. `scripts/autolock-smoke.mjs` turns timers off for exactly this reason.
> - **Back-to-back Playwright smokes against a dev server flake.** Cool down 15-20s between them and **re-run a failure in isolation before believing it**. `standings-smoke` and `host-lifecycle-smoke` have both failed in a batch and passed 2-3 times alone.
> - **Some smokes import workspace TS and need `node_modules/.bin/jiti`, not bare `node`** (`tier-flow-smoke`, `load-test`, `wavelength-smoke`, ...). Running them with `node` fails with a module-resolution error that has nothing to do with the code under test.
> - **`load-test`'s "submissions per round" is noisy and nothing asserts on it.** It counts how many headless clients happened to observe `round.state === 'open'` before the host locked, so a fast round legitimately shows a low number; an A/B showed the same spread on unchanged code. Judge that harness by `page errors`, `roster sees N of N`, `spawn/join fails` and the idle-presence line.
> - **The CLASP client drops any send while the socket is closed and does not queue it.** Fire-and-forget publishes are simply lost during a blip. Anything memoized on "I already published this" must invalidate on reconnect or it will never send again.
> - **A relay `.on()` made before the socket connects is dropped** (CLASP does not replay it); `onExtra` defers to `onConnect` for you, so use it rather than subscribing another way.
> - **Headless browsers throttle background-tab timers**, so pacing, animation and hold-times are unmeasurable headless. Verify timing-sensitive UI in a real foreground browser.
>
> **Where things live:** blocks in `packages/games/src/blocks/<kind>/`; games in `packages/games/src/games/<id>/` (composed games are ~20 lines; custom-flow games ship their own Host/Player `.vue` plus pure tested rules in `logic.ts`); the generic renderer, scoring, `slides.ts` (results paging) and `autoadvance.ts` (who a round waits on) in `packages/games/src/runtime/`; the **engine** in `packages/engine/src/` (`room.ts` is the state machine, presence, roster publishing and answer withholding; `addresses.ts` is the address scheme; `vue/index.ts` is the binding and drives the host tick). `catalog.ts` is the server-safe mirror of `registry.ts`, kept in sync by a test; an answer-bearing block also needs a `REDACTION_RULES` entry. Shell, editor and catalog in `apps/web`.
>
> **Nothing is currently open as a known defect.** Two things worth knowing rather than doing: a phone asleep across a round boundary can miss that one round and rejoins on wake (accepted; if it ever grates, add a minimum-open floor to auto-advance, do NOT accept answers after a round locks, that was decided against and the reasoning is in `HANDOFF.md`); and the roster is one value carrying the whole room (~14KB at 200 players), so if rooms ever outgrow that, publish a delta rather than throttling. Otherwise the next work is content and depth: more flagship games, the gameshow panel, and the external-plugin sandbox (`docs/external-plugins.md`), which is security-critical and still unbuilt.
