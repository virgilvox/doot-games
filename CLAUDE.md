...
| `packages/games` | `@doot-games/games` | the round blocks (guess/answer/wager/rate/poll/rank/draw/quip/vote/fill/split/bars/buzzer/fibvote/drawvote/hivemind/mostlikely/ballpark/faker/accuse/spotlight/cellar/categories/survey/spectrum/slide/title/collect/photovote, plus the per-player chain/role blocks chainline/doodle/wavelength on the P7 foundation), the generic renderer, scoring knobs, and games as compositions. Authoritative lists: `registry.ts` (games) + `index.ts` (blocks); `catalog.ts` is the server-safe mirror |
