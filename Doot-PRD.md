...
Drawings are handled with size in mind. A drawing is serialized primarily as a compact stroke list (an ordered set of strokes with color, width, and points), which is small, replayable, and animatable, and published under the round's input address with a TTL. Photos are pre-clamped to a byte budget before publish.

**Frame-size offload (Claim-Check).** A single CLASP frame is capped at 65535 bytes (a uint16 length prefix), so any value that aggregates many artifacts (a results "unspool" of every drawing, an anonymized gallery to vote on) can exceed one frame. The engine's relay wrapper handles this transparently: a value whose encoded size exceeds ~58 KB is stored off-relay and replaced by a small reference; subscribers resolve it back before it reaches a game. The bytes are the same data that was already relay-bound (so answer-withholding is unchanged), the objects are private and self-expire via an S3 lifecycle rule, and the app proxies storage with signed PUT/GET (same-origin, no browser CORS or public objects, using only S3 paths already proven on the prod bucket). This is the standard Claim-Check pattern. **See [`docs/ephemeral-blobs.md`](./docs/ephemeral-blobs.md) for the full design, security model, and local/DO setup.** Late joiners receive current media via snapshot (references resolve on read); everything expires on its own.
